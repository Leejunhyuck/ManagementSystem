...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.httpBasic().disable() // rest api 이므로 기본설정 사용안함.
                .csrf().disable() // csrf 보안이 필요없다
                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS) // jwt token으로 인증할것이므로, seccion 비활성화                                                           
                .and().authorizeRequests() // 다음 리퀘스트에 대한 사용권한 체크
                .antMatchers("/*/user/signin", "/*/signin/**", "/*/user/signup", "/*/signup/**", "/social/**","/*/joininfo/**","/*/jobtodo/**","/*/board/**").permitAll() // 가입 및 인증은 모든 사용자 접근가능                                                                                    
                ...
                .addFilterBefore(new JwtAuthenticationFilter(jwtTokenProvider),
                        UsernamePasswordAuthenticationFilter.class); // jwt token 필터를 id/password 인증 필터 전에 선 처리
    }
